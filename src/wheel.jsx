    ...
    >
      ...
      >
        ...
        >
          ...
          {/* ═══ El nombre grabado en el cono ═══
              Va adentro del cilindro y gira con él, como el grabado de una
              mesa de verdad. El truco para que se pueda leer girando es que el
              nombre está REPETIDO alrededor de todo el anillo, separado por
              una rayita: siempre hay una copia derecha, mire uno donde mire.
              Cuántas veces entra se calcula con la vuelta del anillo y el
              largo del nombre; después `textLength` reparte las letras para
              que cierren justo, sin huecos ni pisadas. */}
          {nombre && (() => {
            const cx = WHEEL_SIZE / 2;
            const cy = WHEEL_SIZE / 2;
            const texto = String(nombre).toUpperCase();
            const tam = texto.length <= 10 ? 27 : texto.length <= 16 ? 22 : 18;
            const espaciado = texto.length <= 10 ? 4 : 2.5;
            const rAnillo = (INNER_R - 10) * 0.72;
            const vuelta = 2 * Math.PI * rAnillo;

            const unidad = `${texto}  —  `;
            // Georgia en mayúsculas mide ~0,62 del tamaño por letra.
            const anchoUnidad = unidad.length * (tam * 0.62 + espaciado);
            const veces = Math.max(2, Math.round(vuelta / anchoUnidad));
            const cadena = unidad.repeat(veces);

            // Círculo completo: dos medias vueltas encadenadas.
            const d = `M ${cx - rAnillo} ${cy}`
              + ` A ${rAnillo} ${rAnillo} 0 1 1 ${cx + rAnillo} ${cy}`
              + ` A ${rAnillo} ${rAnillo} 0 1 1 ${cx - rAnillo} ${cy}`;
            ...
            return (
              <>
                ...
                <text
                  ...
                  stroke="rgba(20,10,0,0.8)"
                  strokeWidth={tam * 0.13}
                  strokeLinejoin="round"
                >
                  <textPath href="#aro-nombre-mesa" startOffset="0"
                            textLength={vuelta} lengthAdjust="spacing">
                    {cadena}
                  </textPath>
                </text>
              </>
